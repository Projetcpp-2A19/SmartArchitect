...
MainEquip::MainEquip(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::MainEquip)
{
    ui->setupUi(this);
    ...
    connect(ui->pushButton_ajouter, &QPushButton::clicked, this, &MainEquip::on_pushButton_ajouter_clicked);
    connect(ui->pushButton_Afficher, &QPushButton::clicked, this, &MainEquip::on_pushButton_Afficher_clicked);
    connect(ui->exporter, &QPushButton::clicked, this, &MainEquip::exporterPDF);
    connect(ui->pushButton_trier, &QPushButton::clicked, this, &MainEquip::on_pushButton_trier_clicked);
    //connect(ui->pushButton_4, &QPushButton::clicked, this, &MainEquip::on_pushButton_4_clicked);  // Connect customers_btn_4
    ...
    connect(arduino, &Arduino::idReceived, this, &MainEquip::onIdReceived);
}
...
void MainEquip::on_pushButton_2_clicked()
{
    // Placeholder for future functionality
}

void MainEquip::on_pushButton_3_clicked()
{
    // Placeholder for future functionality
}

void MainEquip::on_pushButton_4_clicked()
{
    MainClient *dialog = new MainClient(this);
    dialog->exec();
}

void MainEquip::on_pushButton_ajouter_clicked()
{
    ...
    QDate datedachat = ui->dateEdit_datedachat->date();
    QString caracteristiques = ui->lineEdit_caracteristiques->text();

    QRegularExpression nomRegex("^[A-Za-z\\s]{3,}$");
    QRegularExpression typeRegex("^[A-Za-z\\s]+$");
    QRegularExpression caractRegex("^.{5,}$");
    ...
    equipement e(-1, nom, quantite, type, datedachat, budget, caracteristiques);
    e.setCode_barre(code_barre);
...
void MainEquip::on_pushButton_Afficher_clicked()
{
...
void MainEquip::on_pushButton_supprimer_clicked()
{
...
void MainEquip::on_pushButton_modifier_clicked()
{
    ...
    if (query.exec() && query.next()) {
        ...
        QRegularExpressionValidator nomValidator(QRegularExpression("^[A-Za-z\\s]{3,}$"), &nomEdit);
        nomEdit.setValidator(&nomValidator);

        QRegularExpressionValidator quantiteValidator(QRegularExpression("^[1-9][0-9]*$"), &quantiteEdit);
        quantiteEdit.setValidator(&quantiteValidator);

        QRegularExpressionValidator typeValidator(QRegularExpression("^[A-Za-z\\s]+$"), &typeEdit);
        typeEdit.setValidator(&typeValidator);

        QRegularExpressionValidator budgetValidator(QRegularExpression("^[1-9][0-9]*$"), &budgetEdit);
        budgetEdit.setValidator(&budgetValidator);

        QRegularExpressionValidator caracteristiquesValidator(QRegularExpression("^.{5,}$"), &caracteristiquesEdit);
        ...
        if (dialog.exec() == QDialog::Accepted) {
            equipement e(id, nomEdit.text(), quantiteEdit.text().toInt(), typeEdit.text(),
...
void MainEquip::on_pushButton_trouver_clicked()
{
...
void MainEquip::on_commandLinkButton_chatbot_clicked()
{
    chatbot chatbot;
    chatbot.exec();
}

void MainEquip::exporterPDF()
{
...
void MainEquip::on_pushButton_trier_clicked()
{
    QSqlQuery query;
...
void MainEquip::afficherCodeBarreZoom(QPixmap pixmap)
{
    ...
    zoomDialog->showMaximized();
    zoomDialog->exec();
}

bool MainEquip::eventFilter(QObject* watched, QEvent* event)
{
    if (event->type() == QEvent::MouseButtonPress) {
        ...
        if (label) {
            ...
            imageLabel->setPixmap(fullPixmap.scaled(800, 600, Qt::KeepAspectRatio, Qt::SmoothTransformation));
