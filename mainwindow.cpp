    ...
    , gProjett(nullptr) // Initialize gProjett to nullptr
{
    ...
    connect(ui->products_btn_3, &QPushButton::clicked,
            this, &MainWindow::on_products_btn_3_clicked);
    connect(ui->customers_btn_5, &QPushButton::clicked,
            this, &MainWindow::on_customers_btn_5_clicked); // Connect customers_btn_5
    ...
    connect(ui->tableView, &QTableView::clicked, this, &MainWindow::fillFieldsFromSelectedRow);
}
...
void MainWindow::on_btnModifier_clicked() {
    ...
    bool idOk;
    int employeeId = ui->lineEditEmployeeId->text().toInt(&idOk);
    if (!idOk || employeeId <= 0) {
    ...
    QString position = ui->lineEditPosition->text().trimmed();
    if (position.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "La position ne peut pas être vide !");
        return;
    }

    bool salaryOk;
    double salaryValue = ui->lineEditSalary->text().toDouble(&salaryOk);
    if (!salaryOk || salaryValue <= 0) {
        QMessageBox::warning(this, "Erreur", "Salaire invalide !");
        return;
    }
    QString salary = QString::number(salaryValue, 'f', 2); // Format à 2 décimales

    // Récupération de la date comme texte brut
    QString hireDate = ui->dateEditHireDate->text();



    // Modification
    if (emp.updateEmployee(employeeId, lastName, firstName, email, password,
...
{
    MainProjet *dialog = new MainProjet(this);
    dialog->exec();
}

void MainWindow::on_customers_btn_5_clicked()
{
    GProjett *dialog = new GProjett(this);
    dialog->exec();
}
void MainWindow::fillFieldsFromSelectedRow(const QModelIndex &index) {
    if (!index.isValid()) return;

    QAbstractItemModel *model = ui->tableView->model();
    if (!model) return;

    // Remplir tous les champs avec setText() uniquement
    ui->lineEditEmployeeId->setText(model->data(model->index(index.row(), 0)).toString());
    ui->lineEditLastName->setText(model->data(model->index(index.row(), 1)).toString());
    ui->lineEditFirstName->setText(model->data(model->index(index.row(), 2)).toString());
    ui->lineEditEmail->setText(model->data(model->index(index.row(), 3)).toString());
    ui->lineEditPosition->setText(model->data(model->index(index.row(), 4)).toString());
    ui->lineEditSalary->setText(model->data(model->index(index.row(), 5)).toString());

    // Solution avec setText() pour le QDateEdit
    QString hireDateText = model->data(model->index(index.row(), 6)).toString();
    QLineEdit *dateLineEdit = ui->dateEditHireDate->findChild<QLineEdit*>();
    if (dateLineEdit) {
        dateLineEdit->setText(hireDateText);
    }

    ui->lineEditPassword->setText(model->data(model->index(index.row(), 7)).toString());
}
