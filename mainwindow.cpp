...
#include "mainarch.h"  // Include MainArch here
#include <QMessageBox>
#include <QSqlError>
...
MainWindow::MainWindow(QWidget *parent)
    ...
    , mainArch(nullptr)  // Initialize mainArch to nullptr
{
    ui->setupUi(this);
    setupRFIDInterface();
    ...
    connect(ui->home_btn_5, &QPushButton::clicked,
            this, &MainWindow::on_home_btn_5_clicked);  // Connect home_btn_5
...
{
    delete mainArch;  // Clean up mainArch
    delete ui;
}
...
void MainWindow::updateTableView()
{
    ...
    if (newModel) {
        delete proxyModel->sourceModel(); // Clean up the old model
...
void MainWindow::on_btnTriSalaire_clicked()
{
    ...
    if (newModel) {
        ...
        proxyModel->setSourceModel(newModel);
        ui->tableView->setModel(proxyModel);
        ui->tableView->sortByColumn(6, Qt::AscendingOrder); // 6 = index colonne salaire
...
// Inside MainWindow (when opening MainArch)
void MainWindow::on_home_btn_5_clicked()
{
    MainArch *dialog = new MainArch(this);  // Pass 'this' as parent
    dialog->exec();  // Show as modal (blocking)
    // Or use dialog->show() for non-modal
}
