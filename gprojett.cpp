...
void GProjett::on_btnModifier_clicked() {
    ...
    QString status = ui->comboBox_status->currentText();
    // Use existing estimated_cost and actual_cost from the table (or update if needed)
    double estimated_cost = proxyModel->data(proxyModel->index(index.row(), 7)).toDouble();  // Colonne 7 = ESTIMATED_COST
    double actual_cost = proxyModel->data(proxyModel->index(index.row(), 8)).toDouble();    // Colonne 8 = ACTUAL_COST

    ProjetManager p(id, nom, budget, debut, fin, client, status, estimated_cost, actual_cost);
    if (p.modifierProjet()) {
