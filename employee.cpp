...
bool Employee::ajouterEmploye() {
    ...
    checkQuery.prepare("SELECT COUNT(*) FROM EMPLOYEE WHERE ID_EMPLOYE = :id");
    ...
    QSqlQuery query(db);
    query.prepare("INSERT INTO EMPLOYEE(ID_EMPLOYE, NOM, PRENOM, EMAIL, MDP, POSTE, SALAIRE, DATEEMBOCHE) "
...
bool Employee::modifierEmploye(int ID_EMPLOYE, const QString &nom, const QString &email) {
    ...
    QSqlQuery query(db);
    query.prepare("UPDATE EMPLOYEE SET NOM = :nom, EMAIL = :email WHERE ID_EMPLOYE = :id");
...
QSqlQueryModel* Employee::trierEmployesParSalaireCroissant() {
    ...
    QSqlQueryModel *model = new QSqlQueryModel();
    QString queryStr = "SELECT * FROM EMPLOYEE ORDER BY TO_NUMBER(SALAIRE) ASC";
...
QSqlQueryModel* Employee::rechercherEmployee(const QString &idOuNom) {
    ...
    if (isNumber) {
        // Recherche par ID
        model->setQuery(QString("SELECT * FROM EMPLOYEE WHERE ID_EMPLOYE = %1").arg(id), db);
    } else {
        // Recherche par nom
        QString queryStr = "SELECT * FROM EMPLOYEE WHERE UPPER(NOM) LIKE UPPER('%"+idOuNom+"%') OR UPPER(PRENOM) LIKE UPPER('%"+idOuNom+"%')";
        model->setQuery(queryStr, db);
    }
...
QMap<QString, int> Employee::getStatsByPoste() {
    ...
    QSqlQuery query(db);
    query.prepare("SELECT POSTE, COUNT(*) FROM EMPLOYEE GROUP BY POSTE");
