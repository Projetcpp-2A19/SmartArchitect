...
bool Employee::ajouterEmploye() {
    ...
    QSqlQuery query(db);
    query.prepare("INSERT INTO EMPLOYE (ID_EMPLOYE, NOM, PRENOM, EMAIL, MDP, POSTE, SALAIRE, DATEEMBOCHE) "
...
bool Employee::modifierEmploye(int ID_EMPLOYE, const QString &nom, const QString &email) {
    ...
    QSqlQuery query(db);
    query.prepare("UPDATE EMPLOYE SET NOM = :nom, EMAIL = :email WHERE ID_EMPLOYE = :id");
...
QSqlQueryModel* Employee::afficherEmployes() {
    ...
    model->setHeaderData(7, Qt::Horizontal, QObject::tr("DATEEMBOCHE"));
...
QSqlQueryModel* Employee::trierEmployesParSalaireCroissant() {
    ...
    QSqlQueryModel *model = new QSqlQueryModel();
    QString queryStr = "SELECT * FROM EMPLOYE ORDER BY TO_NUMBER(SALAIRE) ASC";
...
QSqlQueryModel* Employee::rechercherEmployee(const QString &idOuNom) {
    ...
    if (isNumber) {
        // Recherche par ID
        model->setQuery(QString("SELECT * FROM EMPLOYE WHERE ID_EMPLOYE = %1").arg(id), db);
    } else {
        // Recherche par nom
        QString queryStr = "SELECT * FROM EMPLOYE WHERE UPPER(NOM) LIKE UPPER('%" + idOuNom + "%') OR UPPER(PRENOM) LIKE UPPER('%" + idOuNom + "%')";
        model->setQuery(queryStr, db);
    }
